...
import cv2
import json
from database.classes.database import DBAccess

bd = DBAccess()
bd.create_connection()
bd.create_cursor()
bd.select_database("brands")
columnas = ["video_name", "det_number", "time", "score", "size", "position"]
table = "logos"
placeholder = "%s, %s, %s, %s, %s, %s"
query = f"INSERT INTO {table} ({', '.join(columnas)}) VALUES ({placeholder})"
...
def save_data(detection):
    params = list(detection.values())
    bd.set_data(query, params)
